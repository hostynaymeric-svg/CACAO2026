	...
	public FenetrePrincipale(String[] args) {
		...
		if (args.length==0) { // Filiere par defaut
			...
			if (estSeed) {
				...
				IActeur[] equipes = { new Producteur1(), new Producteur2(), new Producteur3(), new Transformateur1(), new Transformateur2(), new Transformateur3(), new Distributeur1(), new Distributeur2(), new abstraction.eq9Distributeur2.Distributeur2(), new Romu()};
				int i=0; 
		...
		for (IActeur acteur : acteurs) {
			if (!(acteur instanceof Banque)) {
				...
				for (Variable i : indicateurs){
					...
					Filiere.LA_FILIERE.getBanque().communiqueCryptos(controlJTextField);
